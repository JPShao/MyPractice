...
class LoadingView @JvmOverloads constructor(
    ...
    private var mOuterCircleRadius: Int
    private val mOuterCircleColor: Int
    private var mInnerTriangleRadius: Int
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        //判断外圆的半径
        mOuterCircleRadius = Math.min(
            mOuterCircleRadius.toFloat(),
            (Math.min(
                w - paddingRight - paddingLeft,
                h - paddingTop - paddingBottom
            ) - 4 * mPaint!!.strokeWidth) / 2
        ).toInt()
        if (mOuterCircleRadius < 0) {
            mStrokeWidth =
                Math.min(w - paddingRight - paddingLeft, h - paddingTop - paddingBottom) / 2
            mOuterCircleRadius =
                Math.min(w - paddingRight - paddingLeft, h - paddingTop - paddingBottom) / 4
        }
        ...
        //判断内圆的半径大小
        mInnerTriangleRadius =
            if (mInnerTriangleRadius < mOuterCircleRadius) mInnerTriangleRadius else 3 * mOuterCircleRadius / 5
        ...
        mPath!!.moveTo(
            centerX - mInnerTriangleRadius / 2,
            (centerY - Math.sqrt(3.0) * mInnerTriangleRadius / 2).toFloat()
        )
        ...
        mPath!!.close()
        mRotateCenter!![measuredWidth / 2.toFloat()] = measuredHeight / 2.toFloat()
        mRoundRectF!!.left = 0f
        mRoundRectF!!.top = 0f
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        setMeasuredDimension(
            measureSize(widthMeasureSpec, 140),
            measureSize(heightMeasureSpec, 140)
        )
    }
    ...
    private fun measureSize(measureSpec: Int, defaultSize: Int): Int {
        ...
        when (specMode) {
            MeasureSpec.EXACTLY -> resultSize = specSize
            MeasureSpec.AT_MOST, MeasureSpec.UNSPECIFIED -> resultSize =
                Math.min(specSize, resultSize)
        }
        return resultSize
    }
    ...
    private class MyHandler(loadingView: LoadingView) : Handler() {
        ...
        override fun handleMessage(msg: Message) {
            if (mLoadingViewWeakReference.get() != null) {
                when (msg.what) {
                    REFRESH_VIEW -> mLoadingViewWeakReference.get()!!
                        .postInvalidate()
                }
            }
        }
    ...
    init {
        ...
        mOuterCircleRadius =
            typedArray.getDimensionPixelSize(R.styleable.LoadingView_outerCircleRadius, 50)
        mOuterCircleColor = typedArray.getColor(R.styleable.LoadingView_outerCircleColor, -0xdd74de)
        mInnerTriangleRadius =
            typedArray.getDimensionPixelSize(R.styleable.LoadingView_innerTriangleRadius, 25)
        mInnerTriangleColor =
            typedArray.getColor(R.styleable.LoadingView_innerTriangleColor, -0xdd74de)
